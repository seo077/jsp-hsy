...
public class ClothesDAO {
	...
	// 옷 전체 가져오기
	public ArrayList<ClothesDTO>getClothes(){
		...
		try {
			...
			while(rs.next()) {
				...
				int cnt = rs.getInt(6);
				int temperature = rs.getInt(7);
				String type = rs.getString(8);
				
				ClothesDTO clothe = new ClothesDTO(code, size, name, img, price, cnt, temperature, type);
				clothes.add(clothe);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ClothesDTO getClothe(int code){
		...
		try {
			...
			while(rs.next()) {
				...
				clothe = new ClothesDTO(code, size, name, img, price, cnt, temperature, type);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	// 옷 등록
	public int writeClothe(String size,String name,String img,int price,int cnt,int temperature,String type) {
